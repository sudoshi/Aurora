/**
 * Aurora <-> OHIF Measurement Bridge + Viewport Resize Fix
 *
 ...
 *    them to the Aurora parent frame via window.postMessage.
 *
  ...
  function setupBridge() {
    ...
    var checkInterval = setInterval(function () {
      ...
      try {
        ...
        console.log('[Aurora Bridge] Measurement bridge active');
      } catch (e) {
    ...
    var resizeInterval = setInterval(function () {
      ...
      for (var i = 0; i < canvases.length; i++) {
        if (canvases[i].width > 0 && canvases[i].height > 0) {
          console.log('[Aurora Bridge] Viewport rendered (' +
