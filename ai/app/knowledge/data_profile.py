"""
DataProfileService — clinical data coverage profiling and gap detection.

Analyses the clinical schema for data completeness, domain density,
temporal coverage, and produces human-readable gap warnings.

Aurora uses a clinical schema with tables for conditions, medications,
procedures, lab results, and vital signs mapped from various source formats.
"""
...
class DataProfileService:
    """Profile clinical data coverage and detect data gaps."""

    def __init__(
        self,
        engine: Engine,
        redis_client: Any,
        clinical_schema: str = "clinical",
    ) -> None:
    ...
    def get_patient_count(self) -> int:
        """Return the total number of patients in the clinical schema."""
        sql = text(f"SELECT COUNT(*) FROM {self.clinical_schema}.patients")
        try:
            with self.engine.connect() as conn:
                result = conn.execute(sql)
                return int(result.scalar() or 0)
        except Exception:
            logger.exception("get_patient_count failed")
            return 0

    def get_temporal_coverage(self) -> dict[str, Any]:
        """Return the earliest and latest clinical record dates."""
        sql = text(
            f"SELECT MIN(recorded_date) AS min_date,"
            f"       MAX(recorded_date) AS max_date"
            f" FROM {self.clinical_schema}.conditions"
        )
        try:
            with self.engine.connect() as conn:
                result = conn.execute(sql)
    ...
    def get_domain_density(self) -> list[dict[str, Any]]:
        ...
        for table in _DOMAIN_TABLES:
            sql = text(f"SELECT COUNT(*) FROM {self.clinical_schema}.{table}")
            try:
                with self.engine.connect() as conn:
    ...
    def detect_data_gaps(
        ...
        if patient_count == 0:
            warnings.append(
                DataGapWarning(
                    gap_type="empty_patients",
                    domain="patients",
                    severity="critical",
                    message="Patients table is empty — no patients loaded",
                )
            )
            return warnings
        ...
        for entry in domain_density:
            ...
            records_per_patient = count / patient_count
    ...
    def get_profile_summary(self) -> dict[str, Any]:
        """Return a complete clinical data profile dictionary."""
