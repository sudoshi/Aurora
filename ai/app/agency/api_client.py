"""Agency API client — authenticated async HTTP client for Aurora Laravel API calls.

Makes Bearer-token-authenticated requests to the Aurora Laravel API on behalf of
...
class AgencyApiClient:
    """Async HTTP client for Aurora Laravel API calls made by the agency module.

    Parameters
    ----------
    base_url:
        Root URL of the Aurora Laravel application
        (e.g. ``https://aurora.acumenus.net``).
    ...
    def _build_url(self, path: str) -> str:
        """Resolve a short path to a full ``/api/`` URL."""
        path = path.lstrip("/")
    ...
    async def call(
        ...
        """Execute an authenticated API call against the Aurora Laravel backend.
        ...
        path:
            API path relative to ``/api/`` (leading slash optional),
            e.g. ``cases`` or ``/cases/42``.
        auth_token:
