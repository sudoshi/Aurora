...
Records clinical case patterns, analysis configurations, user corrections, and data
...
class KnowledgeArtifact:
    ...
    ----------
    artifact_type:
        Category of the artifact, e.g. ``"case_pattern"`` or
        ``"analysis_config"``.
    title:
...
class KnowledgeCapture:
    ...
    def capture_case_creation(
        ...
        *,
        ...
        """Capture a clinical case definition as a ``case_pattern`` artifact.
        ...
        disease_area:
            Disease/therapeutic area of the case.
        case_data:
            Full case definition data (conditions, medications, etc.).
        condition_codes:
            Optional list of condition codes (ICD-10, SNOMED, etc.).
        case_summary:
            Optional text summary of the case.
        tags:
        ...
        -------
        KnowledgeArtifact
            The created artifact with ``artifact_type == "case_pattern"``.
        """
        # Build case_data from explicit fields if not provided directly
        resolved_data: dict[str, Any] = case_data or {}
        if condition_codes is not None:
            resolved_data = {**resolved_data, "condition_codes": condition_codes}
        if case_summary is not None:
            resolved_data = {**resolved_data, "case_summary": case_summary}

        summary = f"Clinical case definition for {case_name}"
        if disease_area:
    ...
    def capture_analysis_completion(
        ...
        *,
        ...
        study_design:
            Study design type, e.g. ``"patient_analysis"``, ``"risk_assessment"``.
        disease_area:
            Disease/therapeutic area.
        analysis_data:
            Analysis parameters (patient IDs, model settings, etc.).
        tags:
            Optional keyword tags.
        source_conversation_id:
            ID of the Abby conversation that triggered this capture.
    ...
    def capture_data_finding(
        ...
        *,
        ...
        affected_domain:
            Domain or clinical table group affected, e.g. ``"conditions"``.
        affected_tables:
