...
const DRUG_COLORS = [
  "#F0607A", "#60A5FA", "#2DD4BF", "#F59E0B", "#A78BFA",
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] text-[#4A5068] uppercase tracking-wider">Patient</span>
        </div>
        <p className="text-sm text-[#E8ECF4] font-semibold font-mono">
      ...
      <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-[10px] text-[#4A5068] uppercase tracking-wider">Treatments</span>
        </div>
...
function VisualTimeline({ data }: { data: TimelineData }) {
  ...
  if (studies.length === 0) {
    return (
      <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] p-8 text-center text-sm text-[#4A5068]">
  ...
  return (
    <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] p-6 space-y-6">
      ...
      {drug_exposures.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] text-[#4A5068] uppercase tracking-wider mb-2">Treatment Context</p>
      ...
      <div className="space-y-2">
        <p className="text-[10px] text-[#4A5068] uppercase tracking-wider">Imaging Studies</p>
        <div className="relative h-16">
          {/* Axis line */}
          <div className="absolute top-1/2 left-0 right-0 h-px bg-[#222256]" />
          ...
          {studies.map((study) => {
            ...
            return (
              ...
              >
                ...
                {/* Date label */}
                <span className="text-[9px] text-[#4A5068] whitespace-nowrap">
                ...
                {studyMeasurements.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-[#2DD4BF] text-[8px] font-bold text-[#0A0A18] flex items-center justify-center">
          ...
          <tbody className="divide-y divide-[#16163A]">
            {studies.map(study => (
              <tr key={study.id} className="hover:bg-[#16163A] transition-colors">
                <td className="px-4 py-3 text-[#B4BAC8] text-xs">{formatDate(study.study_date)}</td>
                <td className="px-4 py-3">
                  <span
                    className="inline-block rounded px-2 py-0.5 text-[10px] font-semibold"
                    style={{
                      backgroundColor: `${MODALITY_COLORS[study.modality ?? ""] ?? "#7A8298"}18`,
                      color: MODALITY_COLORS[study.modality ?? ""] ?? "#7A8298",
                    }}
                  >
                    {study.modality ?? "--"}
                  </span>
                </td>
                <td className="px-4 py-3 text-[#7A8298] text-xs">{study.body_part_examined ?? "--"}</td>
                <td className="px-4 py-3 text-[#7A8298] text-xs max-w-xs truncate">{study.study_description ?? "--"}</td>
                <td className="px-4 py-3 text-[#B4BAC8] text-xs text-center">{study.num_series}</td>
                <td className="px-4 py-3 text-[#B4BAC8] text-xs text-center">{study.num_images}</td>
...
function DrugExposureTable({ drugs }: { drugs: DrugExposure[] }) {
  if (drugs.length === 0) {
    return (
      <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] p-6 text-center text-sm text-[#4A5068]">
    ...
    <div className="rounded-lg border border-[#1C1C48] bg-[#10102A]">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#16163A]">
            {drugs.map((drug, i) => (
              <tr key={`${drug.drug_concept_id}-${i}`} className="hover:bg-[#16163A] transition-colors">
                <td className="px-4 py-3 text-[#E8ECF4] text-xs font-medium max-w-xs truncate">{drug.drug_name}</td>
                <td className="px-4 py-3 text-[#7A8298] text-xs">{drug.drug_class ?? "--"}</td>
                <td className="px-4 py-3 text-[#B4BAC8] text-xs">{formatDate(drug.start_date)}</td>
                <td className="px-4 py-3 text-[#B4BAC8] text-xs">{formatDate(drug.end_date)}</td>
                <td className="px-4 py-3 text-[#B4BAC8] text-xs font-mono">{drug.total_days}d</td>
              </tr>
            ))}
          </tbody>
        </table>
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-[#F0607A]/30 bg-[#F0607A]/10 p-6 flex items-center gap-3">
        ...
        <p className="text-sm text-[#F0607A]">Failed to load patient timeline: {error.message}</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <button
          ...
          className="text-xs text-[#7A8298] hover:text-[#E8ECF4] transition-colors"
        >
