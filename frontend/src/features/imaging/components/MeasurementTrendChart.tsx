...
const TREND_COLORS = [
  "#2DD4BF", "#60A5FA", "#A78BFA", "#F59E0B", "#F0607A",
...
function TrendTooltip({
  ...
  return (
    <div className="rounded-lg border border-[#2A2A60] bg-[#16163A] px-3 py-2 shadow-lg min-w-[160px]">
      <p className="text-xs text-[#7A8298] mb-1">{label ? formatDate(label) : ""}</p>
      {payload.map((p, idx) => {
        ...
        return (
          <div key={idx} className="flex items-center justify-between gap-3 mt-0.5">
            ...
            <span className="font-mono text-xs text-[#E8ECF4]">
              {p.value.toFixed(1)} {unit}
              {pctChange !== null && (
                <span className={`ml-1.5 text-[10px] ${pctChange > 0 ? "text-[#F0607A]" : pctChange < 0 ? "text-[#2DD4BF]" : "text-[#9D75F8]"}`}>
  ...
  return (
    <div className="rounded-xl border border-[#1C1C48] bg-[#10102A] p-4">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-xs font-semibold text-[#E8ECF4] uppercase tracking-wider">
          {chartTitle}
        </h4>
        <div className="flex items-center gap-3">
          <span className="font-mono text-sm text-[#B4BAC8]">
            {lastValue.toFixed(1)} {unit}
          </span>
          {showPercentChange && percentChange !== null && (
            <span className={`font-mono text-xs px-2 py-0.5 rounded-full ${
              percentChange > 5 ? "bg-[#F0607A]/15 text-[#F0607A]" :
              percentChange < -5 ? "bg-[#2DD4BF]/15 text-[#2DD4BF]" :
              "bg-[#9D75F8]/15 text-[#9D75F8]"
            }`}>
      ...
      <ResponsiveContainer width="100%" height={height}>
        <ComposedChart data={data} margin={{ top: 8, right: 12, bottom: 0, left: 0 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#1C1C48" vertical={false} />
          <XAxis
            dataKey="date"
            tickFormatter={formatDateShort}
            tick={{ fill: "#7A8298", fontSize: 10 }}
            axisLine={{ stroke: "#1C1C48" }}
            tickLine={false}
          />
          <YAxis
            tick={{ fill: "#7A8298", fontSize: 10 }}
          ...
          {seriesNames.map((name, i) => (
            <Line
              ...
              strokeWidth={2}
              dot={{ r: 4, fill: TREND_COLORS[i % TREND_COLORS.length], stroke: "#10102A", strokeWidth: 2 }}
              activeDot={{ r: 6, fill: TREND_COLORS[i % TREND_COLORS.length], stroke: "#10102A", strokeWidth: 2 }}
              connectNulls
            />
          ))}
        </ComposedChart>
      ...
      {seriesNames.length > 1 && (
        <div className="flex flex-wrap gap-3 mt-2 pt-2 border-t border-[#1C1C48]">
          {seriesNames.map((name, i) => (
            <div key={name} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: TREND_COLORS[i % TREND_COLORS.length] }} />
              <span className="text-[10px] text-[#7A8298]">{name}</span>
            </div>
          ))}
        </div>
      )}
...
export function MultiTrendChart({ measurements, height = 200 }: MultiTrendChartProps) {
  ...
  if (measurementTypes.length === 0) {
    return (
      <div className="rounded-xl border border-[#1C1C48] bg-[#10102A] p-6 text-center text-sm text-[#4A5068]">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <h3 className="text-sm font-semibold text-[#E8ECF4]">Measurement Trends</h3>
        <span className="text-[10px] text-[#4A5068] uppercase tracking-wider">
