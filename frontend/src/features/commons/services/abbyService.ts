...
export async function submitFeedback(
  ...
  await apiClient.post("/api/commons/abby/feedback", feedback);
}
...
export async function fetchAbbyHistory(params: {
  ...
  }>("/api/commons/abby/history", { params });
  return data.data;
}
...
export async function listAbbyConversations(): Promise<AbbyConversationSummary[]> {
  ...
  }>("/api/abby/conversations?per_page=20");
...
export async function fetchAbbyConversation(
  ...
  }>(`/api/abby/conversations/${conversationId}`);
