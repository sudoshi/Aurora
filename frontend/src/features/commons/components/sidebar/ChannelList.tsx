...
export function ChannelList({ channels, activeSlug }: ChannelListProps) {
  ...
  const caseChannels = filtered.filter((c) => c.type === "case");
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      {caseChannels.length > 0 && (
        <>
          <SectionLabel>Case Channels</SectionLabel>
          {caseChannels.map((ch) => (
            <ChannelItem
              key={ch.id}
              channel={ch}
  ...
  return (
    <button
      ...
      className={`flex items-center justify-between py-1.5 px-4 text-[13px] transition-all duration-150 ${
        isActive
          ? "border-l-2 border-primary bg-primary/10 text-foreground shadow-[inset_0_0_20px_rgba(13,148,136,0.06)]"
          : hasUnread
