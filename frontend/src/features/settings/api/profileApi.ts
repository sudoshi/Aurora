...
export async function updateProfile(payload: UpdateProfilePayload): Promise<ProfileResponse> {
  const { data } = await apiClient.put<ProfileResponse>("/api/user/profile", payload);
  return data;
}
...
export async function uploadAvatar(file: File): Promise<AvatarResponse> {
  ...
  const { data } = await apiClient.post<AvatarResponse>("/api/user/avatar", formData, {
...
export async function deleteAvatar(): Promise<{ message: string }> {
  const { data } = await apiClient.delete<{ message: string }>("/api/user/avatar");
  return data;
}
