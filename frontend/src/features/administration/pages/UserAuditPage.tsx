...
const ACTION_CONFIG: Record<string, { color: string; icon: LucideIcon; label: string }> = {
  ...
  logout:           { color: "#7A8298", icon: LogOut,    label: "Logout" },
  password_changed: { color: "#F59E0B", icon: KeyRound,  label: "Password Changed" },
  password_reset:   { color: "#F0607A", icon: Shield,    label: "Password Reset" },
...
function StatCard({
  label, value, color = "#B4BAC8", icon: Icon,
}: {
  label: string;
  value: number | string;
  color?: string;
  ...
  return (
    <div className="flex items-center gap-3 rounded-lg border border-[#1C1C48] bg-[#10102A] px-4 py-3">
      <div
...
function ActionBadge({ action }: { action: string }) {
  const cfg = ACTION_CONFIG[action] ?? { color: "#7A8298", icon: Activity, label: action };
  const Icon = cfg.icon;
  return (
    <span
  ...
  return (
    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-[#2A2A60] bg-[#10102A] py-16">
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#16163A]">
        <ScrollText size={24} className="text-[#7A8298]" />
      </div>
      <h3 className="text-lg font-semibold text-[#E8ECF4]">
        {filtered ? "No matching events" : "No audit events yet"}
      </h3>
      <p className="mt-2 max-w-md text-center text-sm text-[#7A8298]">
        {filtered
...
export default function UserAuditPage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#E8ECF4]">User Audit Log</h1>
      ...
      {summary?.top_features && summary.top_features.length > 0 && (
        <div className="rounded-lg border border-[#1C1C48] bg-[#10102A] px-4 py-3">
          <p className="mb-2.5 text-[11px] font-semibold uppercase tracking-wider text-[#4A5068]">
          ...
          <div className="flex flex-wrap gap-2">
            {summary.top_features.map((f) => (
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded border border-[#222256] bg-[#16163A] px-2.5 py-1 text-xs text-[#7A8298] transition-colors hover:border-[#2A2A60] hover:text-[#B4BAC8]"
              >
                <span className="font-['IBM_Plex_Mono',monospace]">{f.feature}</span>
                <span className="text-[10px] text-[#4A5068]">x{f.count}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {isFiltered && (
          <button
            ...
            className="text-sm text-[#4A5068] transition-colors hover:text-[#7A8298]"
          >
            Clear all
          </button>
        )}
            ...
            <thead>
              <tr className="bg-[#16163A]">
                {["Time", "User", "Action", "Feature", "IP Address"].map((h) => (
                  <th
                    key={h}
                    className="px-4 py-2.5 text-left text-[11px] font-semibold uppercase tracking-wider text-[#7A8298]"
                  >
                    {h}
                  </th>
                ))}
              ...
              {filtered.map((entry, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    {entry.user_name ? (
                      <div>
                        <p className="text-sm font-medium text-[#B4BAC8]">{entry.user_name}</p>
                        <p className="font-['IBM_Plex_Mono',monospace] text-xs text-[#4A5068]">
                          {entry.user_email}
                        </p>
                      </div>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    {entry.feature ? (
                      <button
                        ...
                        className="font-['IBM_Plex_Mono',monospace] text-xs text-[#7A8298] transition-colors hover:text-[#2DD4BF]"
                      >
                        {entry.feature}
                      </button>
                    ) : (
