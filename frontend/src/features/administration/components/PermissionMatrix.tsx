/**
 * PermissionMatrix -- shows all roles x all permissions in a grid.
...
import type { Role, PermissionsByDomain } from "../api/adminApi";
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-4">
        <p className="text-sm text-[#7A8298]">
          Click cells to toggle permissions. Row headers apply across all roles.
          Column headers grant/revoke all for a role.
        </p>
        {dirty.size > 0 && (
          <button
            type="button"
            onClick={saveAll}
            className="inline-flex items-center gap-2 rounded-lg bg-[#2DD4BF] px-4 py-1.5 text-sm font-medium text-[#0A0A18] transition-colors hover:bg-[#26B8A5] shrink-0"
          >
      ...
      {/* Matrix table */}
      <div className="overflow-x-auto rounded-lg border border-[#1C1C48] bg-[#10102A]">
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-[#1C1C48] bg-[#16163A]">
              <th className="sticky left-0 z-10 w-48 bg-[#16163A] px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider text-[#4A5068]">
                Permission
              </th>
              {editableRoles.map((r) => (
                <th
                  key={r.id}
                  className="min-w-[8rem] cursor-pointer px-2 py-2.5 text-center transition-colors hover:bg-[#1C1C48]"
                  onClick={() => toggleColumn(r.name)}
                  title={`Toggle all permissions for ${r.name}`}
                >
                  <div className="font-semibold text-[#E8ECF4]">{r.name}</div>
                  <div className="mt-0.5 font-normal text-[#4A5068]">
                    {matrix[r.name]?.size ?? 0} perms
                  </div>
                  {dirty.has(r.name) && (
                    <div className="mt-0.5">
                      {saving.has(r.name) ? (
                        <span className="inline-flex items-center gap-1 text-[#7A8298]">
          ...
          <tbody>
            {allDomains.map(([domain, perms]) => (
              <Fragment key={domain}>
                ...
                <tr
                  className="cursor-pointer border-t border-[#1C1C48] bg-[#16163A]/60 transition-colors hover:bg-[#16163A]"
                  onClick={() => toggleDomainRow(perms)}
                  title={`Toggle all ${domain} permissions across all roles`}
                >
                  <td className="sticky left-0 z-10 bg-[#16163A] px-3 py-1.5 font-semibold capitalize text-[#B4BAC8]">
                    {domain}
                  </td>
                  {editableRoles.map((r) => {
                    ...
                    return (
                      <td key={r.id} className="px-2 py-1.5 text-center">
                        {domOn ? (
                          ...
                          <Minus className="mx-auto h-3.5 w-3.5 text-[#7A8298]" />
                        ) : (
                          <X className="mx-auto h-3.5 w-3.5 text-[#F0607A]/40" />
                        )}
                      </td>
                    );
                  })}
                ...
                {perms.map((perm) => {
                  ...
                  return (
                    <tr
                      key={perm.name}
                      className="border-t border-[#1C1C48]/50 transition-colors hover:bg-[#16163A]/30"
                    >
                      <td
                        className="sticky left-0 z-10 cursor-pointer bg-[#10102A] px-3 py-1 transition-colors hover:bg-[#16163A]"
                        onClick={() => toggleRow(perm.name)}
                        title={`Toggle ${perm.name} for all roles`}
                      >
                        <span className="pl-3 font-mono text-[#7A8298]">{action}</span>
                      </td>
                      {editableRoles.map((r) => {
                        ...
                        return (
                          <td
                            key={r.id}
                            className="cursor-pointer px-2 py-1 text-center transition-colors hover:bg-[#16163A]/60"
                          ...
                          >
                            {on ? (
                              ...
                              <X className="mx-auto h-3 w-3 text-[#F0607A]/35" />
                            )}
                          </td>
                        );
                      })}
